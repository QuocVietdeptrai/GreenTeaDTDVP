        ...
        <section class="products">
            ...
            <div class="box-container">
                ...
                ?>
                            <form method="post" action="" class="box">
                                ...
                                <div class="flex">
                                    <p class="price">Giá: <span><?= number_format($fetch_products['price'], 0, '.', '.'); ?>đ</span></p>
                                ...
                                <p class="sub-total">Tổng tiền: <span><?= number_format($sub_total = ($fetch_cart['qty'] * $fetch_products['price']), 0, '.', '.'); ?>đ</span></p>
                                <button type="submit" name="delete_item" class="btn" onclick="return confirm('delete this item')">Xóa</button>
                            </form>
                <?php
                            $grand_total += $sub_total;
                        } else {
                            // Xóa sản phẩm khỏi giỏ hàng nếu không còn hoạt động
                            $delete_inactive_product = $conn->prepare("DELETE FROM `cart` WHERE id = ?");
                            $delete_inactive_product->execute([$fetch_cart['id']]);
                        }
                    }
                } else {
            ...
            ?>
                <div class="cart-total">
                    <p>Tổng tiền thanh toán: <span><?= number_format($grand_total, 0, '.', '.') ?>đ</span></p>
                    <div class="button">
